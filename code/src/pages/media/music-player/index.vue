  ...
  <div class="page">
    <div class="music-box">
      ...
      <div class="right-control">
        <div class="min" title="音量-" @click="onVolume(-0.1)"></div>
        <div class="progress">
          <div class="bar" :style="{ width: control.volume + '%' }"></div>
        </div>
        <div class="max" title="音量+" @click="onVolume(0.1)"></div>
          ...
          <div class="list">
            ...
            >
              <span class="w">
                {{ item.lyric }}
                <span
                  v-if="index == lrcObj.active"
                  :key="index"
                  class="c"
                  :style="{ animationPlayState: control.status == 'paused' ? 'paused' : '' }"
                  >{{ item.lyric }}</span
                >
              </span>
            </div>
...
const audios = [
  '人为情叹 - 阿悠悠.mp3',
  '南北组合 吉萍 - 你曾是我炫耀的资本.ogg',
  '小少焱-上下五千年.mp3',
  '就是南方凯 - 离别开出花.mp3',
  '李驰- - 吻得太逼真DJ（弹棉花的小花）.mp3',
  '程响 - 可能.ogg',
  '走卒(戏腔版)-昊轩京剧-吴昊.128.mp3',
  '邵帅 - 你是人间四月天.ogg'
]
...
const findLyricIndex = () => {
  ...
  let currentTime = mysong.value.currentTime || 0
  const index = lrcObj.lrc.findIndex(
    it => it.tsp <= currentTime && currentTime <= it.tsp + it.duration
  )
...
const draw = (datas, maxValue, max = 100) => {
  ...
  // 内圈半径
  const cArc = parseInt((arc * 0.5).toString())
  // 可用最长度
  const maxLen = parseInt((arc * 0.99 - cArc).toString())
  // 能量柱个数,不能大于数组长度
  ...
  for (let i = 0; i < count; i++) {
    ...
    // 旋转
    context.rotate((i * Math.PI * 2) / count - Math.PI * 0.5)
...
const bg = `url(${import.meta.env.VITE_GIT_OSS}/imgs/common/music-bg.jpg)`
const dark = `url(${import.meta.env.VITE_GIT_OSS}/imgs/common/sk-dark.png)`
</script>
